...
function setToArray(mySet) {
  ...
  let filterArr = arr.filter((value) => typeof value !== "number");

  let sortArr = filterArr.sort();
...
function iterateSet(set) {
  let iterKey = set.keys();

  for (let key of iterKey) {
    console.log(key);
  }

  let iterValue = set.values();
  for (let value of iterValue) {
    console.log(value);
  }

  let iterEntries = set.entries();
  for (let entries of iterEntries) {
    console.log(entries);
  }
  // Використовуємо методу keys для отримання ітератор а ключів
